...
namespace 毕业生管理系统
{
    public partial class 毕业申报通过情况 : Form
    {
        public 毕业申报通过情况()
        {
            InitializeComponent();
        ...
        private void 毕业申报通过情况_Load(object sender, EventArgs e)
        {
            ...
            if (LoginMainFrom.user.userIdenty == 1)
            {
                ...
                string sqlStr = "select stu.s_no,stu.s_name,cl.class_name,pro.pro_name,ac.ac_name,t.t_name,stu.check_state from student stu left join class cl on stu.s_class=cl.class_no left join profession pro on stu.s_pro=pro.pro_no left join academy ac on cl.s_academy=ac.ac_no left join teacher t on cl.t_no=t.t_no where cl.t_no=" + LoginMainFrom.user.userID + " and stu.check_state='已通过审核'";
            ...
            if (LoginMainFrom.user.userIdenty == 2)
            {
                ...
                string sqlStr = "select stu.s_no,stu.s_name,cl.class_name,pro.pro_name,ac.ac_name,t.t_name,stu.check_state from student stu left join class cl on stu.s_class=cl.class_no left join profession pro on stu.s_pro=pro.pro_no left join academy ac on cl.s_academy=ac.ac_no left join teacher t on cl.t_no=t.t_no where cl.b_no=" + LoginMainFrom.user.userID + " and stu.check_state='已通过审核'";
                dataGridView1.DataSource = MysqlWays.checkData(sqlStr).Tables[0];
            ...
            if (LoginMainFrom.user.userIdenty == 3)
            {
                ...
                //添加未提交的学生
                string sqlStr = "select stu.s_no,stu.s_name,cl.class_name,pro.pro_name,ac.ac_name,t.t_name,stu.check_state from student stu left join class cl on stu.s_class=cl.class_no left join profession pro on stu.s_pro=pro.pro_no left join academy ac on cl.s_academy=ac.ac_no left join teacher t on cl.t_no=t.t_no where stu.check_state='已通过审核'";
                dataGridView1.DataSource = MysqlWays.checkData(sqlStr).Tables[0];
                //班级列表添加成员
